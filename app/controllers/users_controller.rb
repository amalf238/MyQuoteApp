class UsersController < ApplicationController
  ...
  def index
    if logged_in? && is_administrator? # If there is a logged in user and that user is an administrator
    @users = User.all # Then retrieve all users from the database
    elsif logged_in? && !is_administrator? # Otherwise, if there is a logged in standard user

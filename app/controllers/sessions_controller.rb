...
# The create method handles the login process. It attempts to find a user in the database based on the provided email, and, if found, checks the submitted password is correct using Bcrypt's authenticate method and that the user's status is "Active". If all of these are correct, the user's id, fname, and is_admin value are allocated to a session object. Validated users are then redirected to a path that applies to then, i.e. admin_path or userhome_path. If authentication fails, an error flash message is set and the login form re-rendered.
def create
